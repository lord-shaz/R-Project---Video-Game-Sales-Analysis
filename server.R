    ...
    newPlot<-reactive({
       ...
       if(input$maintab=='2'){
        ...
        if(is.null(input$searchPublisher)|| input$searchPublisher==''){
          ...
          total_sales=mydata %>%group_by(Publisher)%>%summarise(total_sales=sum(Global_Sales))%>%arrange(desc(total_sales))
          top_sellers=head(total_sales,n=15)
          games_produced=mydata%>%filter(as.character(Publisher) %in% as.character(top_sellers$Publisher)) %>%count(Publisher)
          total=merge(top_sellers[c('Publisher','total_sales')],games_produced,by='Publisher')
          names(total)[3]='Games_Produced'
          
          total=total%>%gather(key='Count',value='Value',total_sales:Games_Produced)
          top15=ggplot(total,aes(x=Publisher,y=Value,group=Count,fill=Count))+
            geom_bar(stat="identity",position = position_dodge(),width = 0.8)+
            coord_flip()+
            ylab("Copies Sold(In Millions) VS Games Produced")+xlab("")+ggtitle("Top Selling Publishers")+theme_bw()+
            theme(plot.title = element_text(size=rel(2) ),axis.text.x=element_text(size=rel(1.5)),axis.text.y=element_text(size=rel(1.5)),
                  axis.title.x=element_text(size=rel(1.5)),axis.title.y=element_text(size=rel(1.5)),
                  legend.text =element_text(size=rel(1.2)),legend.title = element_text(size=rel(1.5)) )+scale_fill_brewer(palette="Set1")
